(() => {
  ...
  const needlesFor=value=>{
    const q=cleanKeyword(value),base=[norm(q),...q.split(/[\s,，。；;、/]+/).map(norm)].filter(Boolean),extra=[];
    for(const p of base)for(const[k,list]of Object.entries(synonymMap))if(p===norm(k))extra.push(...list.map(norm));
  ...
  egwSearch=function strictEgwSearch(q){
    ...
    return S.egw.map(e=>{
      ...
      if(hit(e.title,needles))s+=100;
      if(hit(e.chapter,needles))s+=130;
      for(const t of e.topics||[])if(hit(t,needles))s+=150;
      if(hit(e.summary,needles))s+=55;
      return {e,s};
    }).filter(x=>x.s>=100).sort((a,b)=>b.s-a.s).map(x=>x.e);
  };

  egwBookSearch=function strictEgwBookSearch(q){
    const n=norm(q),generic=['怀爱伦','怀著','怀师母','预言之灵','ellenwhite'].some(x=>n===norm(x));
    if(!n||generic)return [...S.egwBooks];
    return S.egwBooks.filter(b=>{const t=norm(b.title_cn);return t===n||t.includes(n)});
  };
  ...
  search=async function keywordSearch(q){
    ...
    const first=host.querySelector('.groupTitle[data-kind="egw"], [data-kind="egw"]');
