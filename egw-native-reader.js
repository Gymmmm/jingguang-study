(() => {
  ...
  let index=[],current=null,scrollFrame=0;
  ...
  async function openUrl(url,meta={}){
    ...
    try{
      ...
      current={type:'egw',native_url:url,title,chapter:meta.chapter||'',locator:meta.locator||''};
  ...
  detail.addEventListener('scroll',()=>{if(scrollFrame)return;scrollFrame=requestAnimationFrame(()=>{scrollFrame=0;savePosition()})},{passive:true});
  detail.addEventListener('cancel',savePosition);
  const style=document.createElement('style');style.textContent=`.egwReading{max-width:720px}.egwReading .egwParagraph{margin:0;padding:10px 12px;border-radius:7px;font-family:"Songti SC","STSong","Noto Serif SC",serif;line-height:2.05}.egwReading .egwParagraph+ .egwParagraph{margin-top:2px}.egwSourceRef{color:var(--muted);font-size:.78em}.egwChapterTitle{font-size:16px;color:var(--muted);font-weight:600;margin-top:-4px}@media(max-width:560px){.egwReading .egwParagraph{padding:9px 5px;line-height:1.95}}`;document.head.appendChild(style);
